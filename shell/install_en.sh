...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> Please refer to this tutorial for adding methods, https://github.com/Jacknoms/vauwgj/blob/master/documents/dns_txt.md"
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/Jacknoms/vauwgj/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
customCDNIP() {
	...
	echoContent yellow "# Notes"
	echoContent yellow "\nTutorial address:"
	echoContent skyBlue "https://github.com/Jacknoms/vauwgj/blob/master/documents/optimize_V2Ray.md"
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/Jacknoms/vauwgj/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/Jacknoms/vauwgj/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/Jacknoms/vauwgj/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/Jacknoms/vauwgj/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/Jacknoms/vauwgj/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "Any door unlock details, please check this article [https://github.com/Jacknoms/vauwgj/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
menu() {
	...
	echoContent green "Github:https://github.com/Jacknoms/vauwgj"
