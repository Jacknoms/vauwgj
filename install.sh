...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> 添加方法请参考此教程，https://github.com/Jacknoms/vauwgj/blob/master/documents/dns_txt.md"
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/Jacknoms/vauwgj/${scriptType}/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/Jacknoms/vauwgj/${scriptType}/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/Jacknoms/vauwgj/${scriptType}/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/Jacknoms/vauwgj/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
